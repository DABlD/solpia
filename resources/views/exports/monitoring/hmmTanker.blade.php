		...
		<tr>
			...
			@php
				...
				foreach($obc->document_lc as $doc){
					...
					elseif($doc->type == "BASIC TRAINING FOR OIL AND CHEMICAL TANKER - BTOCT"){
						$btoc = $doc;
					}
					elseif($doc->type == "ADVANCE TRAINING FOR OIL TANKER - ATOT"){
						$atot = $doc;
					}
					elseif($doc->type == "ADVANCE TRAINING FOR CHEMICAL TANKER - ATCT"){
						$atct = $doc;
					}
			...
			<td>{{ $btoc ?( $btoc->expiry_date ? '=DATEVALUE("' . $btoc->expiry_date->format('m/d/Y') . '")' : "UNLIMITED") : "N/A" }}</td>
			<td>{{ $atot ?( $atot->expiry_date ? '=DATEVALUE("' . $atot->expiry_date->format('m/d/Y') . '")' : "UNLIMITED") : "N/A" }}</td>
			<td>{{ $atct ?( $atct->expiry_date ? '=DATEVALUE("' . $atct->expiry_date->format('m/d/Y') . '")' : "UNLIMITED") : "N/A" }}</td>
