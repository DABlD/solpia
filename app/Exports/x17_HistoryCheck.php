...
class x17_HistoryCheck implements FromView, WithEvents, WithDrawings//, ShouldAutoSize
{
    public function __construct($data, $type){
        $data->load('sea_service');

        if(sizeof($data->sea_service)){
            $ss = $data->sea_service->sortByDesc('sign_off')->first();
            $data->last_vessel = $ss->vessel_name;
        }
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle) {
                ...
                $event->sheet->getDelegate()->setTitle('CHARACTER INVESTIGATION FORM', false);
                $event->sheet->getDelegate()->getHeaderFooter()->setOddFooter('&LSMOP-SEHC-03 (Rev 1) 16.08.17');
