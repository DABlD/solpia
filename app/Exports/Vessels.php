...
class Vessels implements FromView, WithEvents//, WithDrawings//, ShouldAutoSize
{
    public function __construct($data, $type){
        $lupc = LineUpContract::where('status', 'On Board')->select('id','vessel_id')->get()->groupBy('vessel_id');

        $total = 0;

        // SORT AND GROUP BY PRINCIPAL
        $data = $data->sortBy('pname')->groupBy('pname');

        // SORT AND GROUP BY TYPE
        foreach($data as $key => $vessels){
            $data[$key] = $vessels->sortBy('type')->groupBy('type');
            $data[$key]['count'] = sizeof($vessels);
            $total += $data[$key]['count'];

            $ctr = 0;
            foreach($vessels as $vessel){
                $vessel['noc'] = isset($lupc[$vessel->id]) ? sizeof($lupc[$vessel->id]) : 0;
                $ctr += $vessel['noc'];
            }

            $data[$key]['totalCrew'] = $ctr;
        }

        // dd($data['KLC SM CO., LTD.']);
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle, $total) {
                // SHEET SETTINGS
                $size = \PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4;
                $event->sheet->getDelegate()->getPageSetup()->setPaperSize($size);
                $event->sheet->getDelegate()->setTitle('Vessels', false);
                ...
                $event->sheet->getDelegate()->setAutoFilter('A2:L2');
                ...
                // $event->sheet->getDelegate()->getColumnDimension('H')->setWidth(2);
                ...
                // SET PRINT AREA
                $event->sheet->getDelegate()->getPageSetup()->setPrintArea('A1:L' . ($total + 2));

                // CUSTOM FONT AND STYLE TO DEFINED CELL
                $event->sheet->getDelegate()->getStyle('A1')->getFont()->setSize(16);
                $event->sheet->getDelegate()->getStyle('A3:A' . ($total + 2))->getFont()->setSize(10);
