...
namespace App\Exports\CrewChange;
...
use App\Models\{Vessel, LineUpContract, ProcessedApplicant, Principal};

class x21_CrewChangePlan implements FromView, WithEvents//, WithDrawings//, ShouldAutoSize
{
    public function __construct($data, $type, $req){
        $principal = Principal::find($req['id']);
        $vessels = Vessel::where('principal_id', $principal->id)->where('status', 'ACTIVE')->get();

        $ids = $vessels->pluck('id')->toArray();
        
        $off = LineUpContract::whereIn('vessel_id', $ids)->where('status', 'On Board')->whereNotNull('reliever')->get();

        $off->load('applicant.user');
        $off->load('rank');
        $off->load('vessel');

        $off->load('pa_reliever.applicant.user');

        $aids = [];

        foreach($off as $crew){
            if($crew->pa_reliever){
                array_push($aids, $crew->pa_reliever->applicant_id);
            }
        }

        $on = ProcessedApplicant::whereIn('vessel_id', $ids)->where('status', 'Lined-Up')->whereNotIn('applicant_id', $aids)->get();

        $ids = array_unique($off->pluck('vessel_id')->merge($on->pluck('vessel_id'))->toArray());

        $off = $off->groupBy('vessel_id');
        $on = $on->groupBy('vessel_id');


        // $on->load('applicant.user');
        // $on->load('rank');
        // $on->load('vessel');
        ...
        // $vids = array_unique(array_merge(array_keys($off->toArray()), array_keys($on->toArray())));

        // $data = [];

        // foreach($vids as $vid){
        //     if(isset($off[$vid])){
        //         foreach($off[$vid] as $crew){
        //             $data[$vid]["off"] = $crew;

        //             if($crew->reliever != "No Reliever"){
        //                 foreach($on[$vid] as $key => $crew2){
        //                     if($crew->reliever == $crew2->applicant->id){
        //                         $data[$vid]["on"] = $crew2;
        //                         unset($on[$vid][$key]);
        //                     }
        //                 }
    ...
    {
        return view('exports.crewChange.' . lcfirst($this->type) . "_" . $this->principal->name, [
            'data' => $this->data,
            'ids' => $this->ids,
            'off' => $this->off,
            'on' => $this->on,
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle) {
                ...
                $event->sheet->getDelegate()->setTitle('교대계획', false);
                ...
                $event->sheet->getDelegate()->getStyle('A2:K150')->getFont()->setSize(10);
                $event->sheet->getDelegate()->getStyle('A1:K150')->getFont()->setName('Calibri');
            },
        ];
    }
