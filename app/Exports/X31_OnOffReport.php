...
class X31_OnOffReport implements FromView, WithEvents, WithDrawings//, ShouldAutoSize
{
    public function __construct($data, $type, $req){
        $on = LineUpContract::where('joining_date', ">=", $req['from'])
                            ->where('joining_date', "<=", $req['to'])
                            ->whereIn('vessel_id', $req['vessels'])
                            ->with('document_lc')
                            ->orderBy('joining_date')
                            ->get();
        $off = LineUpContract::where('disembarkation_date', ">=", $req['from'])
                            ->where('disembarkation_date', "<=", $req['to'])
                            ->whereIn('vessel_id', $req['vessels'])
                            ->with('document_lc')
                            ->orderBy('disembarkation_date')
                            ->get();

        foreach($on as $crew){
            $docs = $crew->document_lc;
            $crew->OEC = null;
    ...
    public function registerEvents(): array
    {
        ...
        $size2 = sizeof($this->data->on) > sizeof($this->data->off) ? sizeof($this->data->on) : sizeof($this->data->off);

        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle, $size2) {
                // SHEET SETTINGS
                $size = \PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4;
                $event->sheet->getDelegate()->getPageSetup()->setPaperSize($size);
                $event->sheet->getDelegate()->setTitle('ON OFF LIST', false);
                ...
                $event->sheet->getDelegate()->getHeaderFooter()->setOddFooter('&LDOC. SMOP-OWMS-25 &CEFFECTIVE DATE 09 AUG 2022 &RREV.0');
                ...
                // COLUMN RESIZE
                $event->sheet->getDelegate()->getColumnDimension('A')->setWidth(26);
                ...
                $event->sheet->getDelegate()->getColumnDimension('K')->setWidth(21);
    ...
    public function drawings()
    {
        ...
        $drawing->setHeight(77);
        $drawing->setWidth(1050);
