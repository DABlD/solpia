...
class X18_EvaluationSheet implements FromView, WithEvents//, WithDrawings//, ShouldAutoSize
{
    public function __construct($data, $type, $req){
        $data->load("sea_service");
        $data->rank2 = Rank::where('id', $req['rid'])->first();
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle) {
                ...
                $event->sheet->getDelegate()->setTitle('Evaluation Sheet', false);
                ...
                $cells[4] = array_merge([
                    'B3:C6', 'D3:H6', 'B7:H7', 'B8:H10', 
                    'B12:C27', 'D12:D27', 'E12:F27', 'G12:G27', 'H12:H27',
                    'B29:C35', 'D29:D35', 'E29:F35', 'G29:G35', 'H29:H35',
                    'B36:C37', 'D36:D37', 'E36:F37', 'G36:G37', 'H36:H37',
                    'B39:C40', 'D39:D40', 'E39:F40', 'G39:G40', 'H39:H40',
                    'B42:H49'
                ]);
                ...
                for($i = 13; $i <= 37; $i++){
                    $temp = $event->sheet->getCellByColumnAndRow(8, $i)->getValue();
                    if($temp < 0){
                        $event->sheet->getDelegate()->getStyle("H$i")->getFont()->getColor()->setARGB("FF0000");
                        $minus += $temp;
                    }
                    elseif($temp > 0){
                        $plus += $temp;
                ...
                if($plus >= 25){
                    $event->sheet->getCellByColumnAndRow(7, 39)->setValue("PASS");
                }
                else{
                    $event->sheet->getCellByColumnAndRow(7, 39)->setValue("REJECT");
                }

                if($minus <= -15){
                    $event->sheet->getCellByColumnAndRow(7, 40)->setValue("REJECT");
                }
                else{
                    $event->sheet->getCellByColumnAndRow(7, 40)->setValue("PASS");
                }
                $event->sheet->getDelegate()->getStyle("G38:G39")->getFont()->getColor()->setARGB("FF0000");

                $event->sheet->getCellByColumnAndRow(8, 39)->setValue($plus);
                $event->sheet->getCellByColumnAndRow(8, 40)->setValue($minus);
                $event->sheet->getDelegate()->getStyle("H39")->getFont()->getColor()->setARGB("FF0000");

                $event->sheet->getCellByColumnAndRow(5, 46)->setValue($plus . " / " . $minus);
                $event->sheet->getCellByColumnAndRow(7, 46)->setValue($plus >= 25 && $minus > -15 ? "Y" : "N");
                ...
                $event->sheet->getDelegate()->getRowDimension(7)->setRowHeight(8);
                $event->sheet->getDelegate()->getRowDimension(8)->setRowHeight(60);
                $event->sheet->getDelegate()->getRowDimension(9)->setRowHeight(36);
                $event->sheet->getDelegate()->getRowDimension(11)->setRowHeight(8);
                $event->sheet->getDelegate()->getRowDimension(14)->setRowHeight(22);
                $event->sheet->getDelegate()->getRowDimension(15)->setRowHeight(22);
                ...
                $event->sheet->getDelegate()->getRowDimension(35)->setRowHeight(30);
                $event->sheet->getDelegate()->getRowDimension(38)->setRowHeight(3);
                $event->sheet->getDelegate()->getRowDimension(41)->setRowHeight(8);
                
                // SET PRINT AREA
                $event->sheet->getDelegate()->getPageSetup()->setPrintArea("B1:H49");
            },
        ];
    }
