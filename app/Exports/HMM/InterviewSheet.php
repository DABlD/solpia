...
class InterviewSheet implements FromView, WithEvents, WithDrawings//, ShouldAutoSize
{
    ...
    {
        $fleet = $this->data->user->fleet;
        $fleet = strtolower(str_replace(' ', '_', $fleet));
        return view('exports.interview.' . $fleet . '.' . $this->principal . '_' . $this->rank, [
            'data' => $this->data,
        ]);
    }
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle) {
                ...
                // VC
                $h[7] = [
                    'B1', 'A9', 'E' . ($this->rows + 2) . ':F' . ($this->rows + 2)
                ];
                ...
                // ALL BORDER THIN
                $cells[0] = array_merge([
                    'A9:I' . ($this->rows + 1), 'A' . ($this->rows + 3) . ':I' . ($this->rows + 12)
                ]);
                ...
                // OUTSIDE BORDER THIN
                $cells[3] = array_merge([
                    'A3:C7', 'E3:I7', 'A' . ($this->rows + 3) . ':I' . ($this->rows + 23),
                    'B1:G1'
                ]);
                ...
                $cells[12] = array_merge([
                    ...
                    'B5:C5',
                    'B6:C6',
                    'C' . ($this->rows + 14) . ':G' . ($this->rows + 14),
                    'C' . ($this->rows + 15) . ':G' . ($this->rows + 15),
                    'C' . ($this->rows + 16) . ':G' . ($this->rows + 16),
                    'C' . ($this->rows + 17) . ':G' . ($this->rows + 17),
                    'C' . ($this->rows + 18) . ':G' . ($this->rows + 18),
                    'C' . ($this->rows + 19) . ':G' . ($this->rows + 19),
                    'E' . ($this->rows + 22) . ':I' . ($this->rows + 22)
                ]);
                ...
                // FOR THE CHECK
                $event->sheet->getDelegate()->getStyle('A1:I' . ($this->rows + 25))->getFont()->setName('Times New Roman');
                $event->sheet->getDelegate()->getStyle('A1:I10')->getFont()->setSize(10);
                $event->sheet->getDelegate()->getStyle('B1')->getFont()->setSize(14);
