...
class HMM implements FromView, WithEvents, WithDrawings//, ShouldAutoSize
{
    public function __construct($applicant, $title = "HMM MLC"){
        ...
        // FOR FLEET C
        if($applicant->vessel->fleet == "FLEET C" && !in_array($applicant->vessel->id, [6072, 5842, 5553, 5842])){
            $mt = true;
    ...
    public function registerEvents(): array
    {
        ...
        if(in_array($this->applicant->vessel->id, [7141, 7517, 4623, 4637, 6072, 5842, 5553, 5842])){
            $mt += 3;
        }
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) use ($borderStyle, $fillStyle, $headingStyle, $title, $mt) {
                ...
                // RICH TEXTS
                if(!in_array($this->applicant->vessel->id, [7108, 7517, 7141, 4637, 4623, 6072, 5842, 5553, 5842])){
                    $cell = "A6";
